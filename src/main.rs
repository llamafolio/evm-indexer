...
async fn fetch_blocks_range(rpc: IndexerRPC, db: IndexerDB,  from: i64, to: i64, batch_size: usize) {
    log::info!("==> Main: Fetching block range from {} to {} with batches of {} blocks", from, to, batch_size);

    let blocks_numbers: Vec<i64> = (from..to).collect();

    for chunk in blocks_numbers.chunks(batch_size) {
        
        log::info!("==> Main: Procesing chunk from block {} to {}", chunk.first().unwrap(), chunk.last().unwrap());
...
async fn main() {
    ...
    let rpc = IndexerRPC::new(&rpc_ws_url, &rpc_http_url).await.expect("Unable to connect to the rpc url");
    ...
    fetch_blocks_range(rpc, db, last_synced_block + 1, last_chain_block, DEFAULT_FETCHER_BATCH_SIZE).await;
